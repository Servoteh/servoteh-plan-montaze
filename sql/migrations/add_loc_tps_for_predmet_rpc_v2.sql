-- ============================================================================
-- LOKACIJE — RPC: loc_tps_for_predmet (v2)
-- ----------------------------------------------------------------------------
-- Verzija 2: dodaje server-side filtere koji se koriste sa novog "Predmet"
-- taba u modulu Lokacije. Ranije je UI fetch-ovao sve redove (do 200) i
-- filtrirao klijentski — sada se filteri primenjuju u bazi pa paginacija
-- (count/limit/offset) radi tačno za suženi skup.
--
-- Novi parametri:
--   • p_tp_no           text  — ILIKE filter na "tp_no" (broj TP, drugi deo
--                                ident_broj-a; npr. "1088" za 7351/1088).
--   • p_drawing_no      text  — ILIKE filter na "wo_broj_crteza".
--   • p_location_filter text  — 'with'    → samo TP-ovi koji IMAJU bar jedan
--                                            aktivan placement (placement_id
--                                            IS NOT NULL u redu)
--                              'without' → samo TP-ovi BEZ placement-a (red
--                                            sa praznim location_*)
--                              NULL/'all' → svi (default).
--
-- Postojeći parametri (p_only_open, p_include_assembled, p_limit, p_offset)
-- se ne menjaju semantički. Stara signature se DROP-uje pa PostgREST resolver
-- nema dvosmislenost — UI poziva samo novu (sa default-ima za ostale args).
--
-- Idempotentno: drop ako postoji bilo koja od starih signatura, pa CREATE
-- nove. Bez izmena drugih objekata.
-- ============================================================================
...
BEGIN
  ...
  filt AS (
    ...
    WHERE
      ...
      -- Nov: filter po prisustvu placement-a (Sa lokacijom / Bez lokacije / Svi).
      AND (
...
COMMENT ON FUNCTION public.loc_tps_for_predmet(bigint, boolean, boolean, text, text, text, int, int) IS
  'Lokacije v2: svi TP-ovi (RN) za jedan Predmet, sa LEFT JOIN-om ka loc_item_placements. '
  'Server-side filteri: p_tp_no/p_drawing_no (ILIKE), p_location_filter (with/without/all). '
